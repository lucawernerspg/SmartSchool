    ...
    def open_form(self, row_id):
        ...
        for i, text in enumerate(entry_texts):  # Exclude Temperature column
            if i == 1:  # Hours column
                ...
                HA = ','.join(str(selected_options[x-1]) for x in selected_options)
        ...
        # Save button
        save_button = tk.Button(form, text="Save", command=lambda: self.save_data(db, row_id, entry_vars, HA, DaysActive, MonthsActive))
        save_button.grid(row=len(labels), columnspan=2, pady=10)

    def save_data(self, db, row_id, entry_vars, HA, DaysActive, MonthsActive):
        # Update the data in the database
        cursor = db.cursor()
        query = "UPDATE Routines SET Name = %s, HoursActive = %s, DaysActive = %s, MonthsActive = %s, Temperature = %s WHERE ID = %s"
        print(query)
        values = [entry_vars[0].get(), HA, DaysActive, MonthsActive, entry_vars[1].get(), row_id]
